...
firebase = firebase.FirebaseApplication("https://codebetweenyouandyourfriends-default-rtdb.firebaseio.com/",None)
...
def enter_room():
    ...
    btn_send= Button(message_window,command=send_data,text="Send",font="arial 13",bg="#d6ca98",fg="black",padx=10,relief=FLAT)
...
def get_data():
    ...
    print(get_your_data)
    bytes_str = bytes.fromhex(get_your_data)
    original = decrypt("AIM",bytes_str)
    print("Original Data" + str(original))
    ...
    if(get_friends_data != None):
        ...
        bytes_str = bytes.fromhex(get_friends_data)
        original = decrypt("AIM",bytes_str)
        final_message = original.decode("Utf-8")
        if(final_message not in last_value):
            print(final_message)
            messgae_text.insert(END,final_message + "\n")
...
btn_start = Button(login_window,text="start", font="arial 13", bg="#cef9f2",fg="black",command=enter_room,relief=FLAT,padx=10)

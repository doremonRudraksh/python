...
label_heading.place(relx=0.05, rely=0.01, anchor=W)
...
label_name = Label(root, text="Select Fruit", bg="orange2", font=("Bahnshrift Light", 15))
...
label_quantity = Label(root, text="Enter Quantity", bg="orange2", font=("Bahnshrift Light", 15))
...
label_show_amount = Label(root,  bg="orange2", font=("Bahnshrift Light", 12))
label_show_amount.place(relx=0.95, rely=0.7 , anchor =E)

label_show_quantity = Label(root,  bg="orange2", font=("Bahnshrift Light", 12))
label_show_quantity.place(relx=0.95, rely=0.8 , anchor =E)

class Juice:
    def __init__(self, juice_name, quantity):
        self.juice = juice_name
        self.quantity = int(quantity)
        self.__cost = 250

    def __calculate_cost(self):
        total_cost = self.quantity * self.__cost
        label_show_amount["text"] ="Yo have to pay " + str(total_cost ) + " USD"
        if(self.juice == "Apple"):
            calorie = self.quantity *52
            fruit_image["image"] = apple

        elif(self.juice == "Mango"):
            calorie = self.quantity *60
            fruit_image["image"] = mango

        elif(self.juice == "Orange"):
...
button = Button(root, text="Total", bg="red3", fg="white", padx=10, pady=1, font = ("Arial", 11, "bold"), relief=FLAT, command=order_juice)
